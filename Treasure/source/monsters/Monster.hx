...
class Monster extends FlxSprite {
	...
	var _speed:Float;
	var _distance:Int;
	public var _spawning:Bool = true;
	...
	public function checkEntity(x, y, mWalls:FlxTilemap, monsters:FlxTypedGroup<monsters.Monster>):Bool {
		...
		for (m in monsters) {
			if (m.alive && Math.abs(m.x - x) < 8 && Math.abs(m.y - y) < 8)
				return (false);
		}
		return (true);
	}
	...
	public function move(mWalls:FlxTilemap, monsters:FlxTypedGroup<monsters.Monster>):Void {
		...
		if (_cooldown && _spawning)
			...
			if (diff_x == 0 && diff_y < 0 && arr[0] != -1)
				_dir = 3;
			else if (diff_x == 0 && diff_y > 0 && arr[2] != -1)
				_dir = 2;
			else if (diff_y == 0 && diff_x < 0 && arr[3] != -1)
				_dir = 1;
			else if (diff_y == 0 && diff_x > 0 && arr[1] != -1)
				_dir = 0;
